...
let $CalculatePlayerTurnEvent = Java.loadClass(
  "net.neoforged.neoforge.client.event.CalculatePlayerTurnEvent"
);
let riftContainerWhitelist = ["minecraft:chest", "minecraft:barrel"];
...
Ingredient.of(/justdirethings:time_crystal.*/)
  .itemIds.toArray()
  .forEach((id) => {
    breakableBlocks.push(id);
  });
Ingredient.of(/ftbstuff:.*/)
  .itemIds.toArray()
  .forEach((id) => {
    breakableBlocks.push(id);
  });
...
ItemEvents.rightClicked((event) => {
  if (
    event.level.dimension.path != "the_rift" ||
...
BlockEvents.rightClicked((event) => {
  ...
  if (
    event.player.mainHandItem?.id == "mekanism:cardboard_box" ||
    event.player.offHandItem?.id == "mekanism:cardboard_box"
  ) {
    event.player.tell(
      Text.translate(["message.rift.cardboardUse"]).darkPurple()
    );
    event.cancel();
  }

  if (
    event.getBlock().getEntity() != null &&
    !riftContainerWhitelist.includes(event.getBlock().id.toString())
  ) {
    event.player.tell(Text.translate(["message.rift.itemUse"]).darkPurple());
    event.cancel();
  }
...
PlayerEvents.tick((event) => {
  ...
  if (!isInRift) {
    var wasInRift = event.player.persistentData.contains("isInRift")
      ? event.player.persistentData.getBoolean("isInRift")
      : false;
    if (wasInRift) {
  ...
  if (event.player.isCreative() || event.player.isSpectator()) return;

  let timer = event.player.persistentData.contains("rift_charge")
    ? event.player.persistentData.getInt("rift_charge")
    : 20 * 5;
      ...
      if (timeLeft % (20 * 60) == 0) {
        ...
        new ImmersiveMessage(
          ...
          Text.translate(
            ...
            minutesLeft,
            minutesLeft == 1
              ? Text.translate("message.rift.timeLeft.minute")
              : Text.translate("message.rift.timeLeft.minutes")
          ).getString()
        )
          .setColor("#AA00AA")
          .setDuration(5)
...
function kickFromRift(player) {
  ...
  player
    .getServer()
    .runCommand(`/execute as ${player.username} run ftbteambases home`);
}
...
function checkBackpack(item, player, doRemove) {
  if (
    item.idLocation.namespace != "sophisticatedbackpacks" ||
    !item.idLocation.path.endsWith("backpack")
  )
    return;
...
function isRiftLoot(item) {
  if (!item.getComponents().has($dc.LORE)) return false;

  const lore = item.getComponents().get($dc.LORE);
  if (lore.lines().isEmpty()) return false;
...
LootJS.modifiers((event) => {
  ...
  event.addTableModifier(/.*/).customAction((context, loot) => {
    ...
    if (attacker.level.dimension.path != "the_rift") return;

    loot.forEach((item) => {
      if (item.id.includes("shulker_box")) return;
