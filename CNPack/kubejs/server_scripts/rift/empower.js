const NotificationToastData = Java.loadClass(
...
ActuallyAdditionsEvents.empower((event) => {
  ...
  let player =
    event.getPlayer() ??
    level.getNearestPlayer(event.pos.x, event.pos.y, event.pos.z, range, false);
...
function chargePlayer(player, recipe) {
  ...
  let playerCharge = player.persistentData.contains("rift_charge")
